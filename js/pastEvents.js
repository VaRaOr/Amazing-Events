...
async function getDataFromAPI() {
    ...
    let eventosPasados = eventos.filter(e => fecha > e.date)

    categoriasXD(eventosPasados)
    ...
    function arrayFiltrado() {
        ...
        if (checkboxSelected.length > 0 && textSearch !== "") {
            checkboxSelected.map(selected => {
                datos.push(...eventosPasados.filter(evento => evento.name.toLowerCase().includes(textSearch.trim().toLowerCase()) && evento.category == selected))

            })
        }
        else if (checkboxSelected.length > 0 && textSearch === "") {
            checkboxSelected.map(selected => {
                datos.push(...eventosPasados.filter(evento => evento.category == selected))
            })
        }
        else if (checkboxSelected.length == 0 && textSearch !== "") {
            datos.push(...eventosPasados.filter(evento => evento.name.toLowerCase().includes(textSearch.trim().toLowerCase())))
        }
        else { datos.push(...eventosPasados) }
...
function categoriasXD(array) {
    ...
    let todasLasCategorias = array.map(eventos => eventos.category) //Recorro el array de eventos y separo de este la propiedad categorias
