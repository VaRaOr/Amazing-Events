let checkboxSelected = [];
let textSearch = "";
let data = []

async function getDataFromAPI() {
    const url = "https://mindhub-xj03.onrender.com/api/amazing";
    const headers = new Headers();
    headers.append("User-Agent", "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36");

    try {
        const response = await fetch(url, {
            method: "GET",
            headers: headers,
            mode: "cors"
        });

        if (!response.ok) {
            throw new Error("Error en la solicitud: " + response.status);
        }

        const data = await response.json();
        return data
    } catch (error) {
        console.log("Error al realizar la petición:", error.message);
    }
}
data = await getDataFromAPI()

function categoriasXD() {
    //ANCHOR CREO LOS CHECKBOX DINAMICOS Y TRABAJO CON ELLOS DESDE EL INICIO DE LA CARGA DEL DOCUMENTO
    let checkboxes = document.getElementById("categorias")

    let todasLasCategorias = data.events.map(eventos => eventos.category) //Recorro el array de eventos y separo de este la propiedad categorias

    const datosArray = new Set(todasLasCategorias) //Dado al recorrer el array anterios me va a dar todas las propiedades, con el metodo set elimino las repetidas y dejo solo el primer elemento encontrado, el resto lo descarta

    let categorias = [...datosArray] //guardo el dato obtenido con el metodo anterior en la letiable categorias

    let imputCheckbox = ""
    categorias.forEach(categorias => {
        imputCheckbox += `
            <div class="form-check form-check-inline">
            <label class="form-check-label">
            <input class="form-check-input" type="checkbox" value="${categorias}"/> ${categorias}
            </label>
            </div>`
    })
    //Creo el template de html para poder incorporarle el valor de cada uno de los elementos recorridos en el metodo forEach
    checkboxes.innerHTML = imputCheckbox //Imprimo el resultado en html
    let id = 1
    data.events.map(eventos => eventos.id = id++)
    console.log(data.events)
}
categoriasXD()

let checkBox = document.querySelectorAll("input[type=checkbox]");
console.log(checkBox)

checkBox.forEach(check => check.addEventListener("click", (event) => {
    let checked = event.target.checked
    if (checked) {
        checkboxSelected.push(event.target.value)
        arrayFiltrado() //aca va una fn que filtra las posibilidades de busqueda
        console.log(checkboxSelected);
    } else {
        checkboxSelected = checkboxSelected.filter(uncheck => uncheck !== event.target.value)
        arrayFiltrado() //aca va una fn que filtra las uncheck
        console.log(checkboxSelected);
    }
}))

let searchinput = document.querySelector("#search")
searchinput.addEventListener("keyup", (event) => {
    textSearch = event.target.value
    arrayFiltrado() //fn xd
})

function arrayFiltrado() {
    let datos = [];
    if (checkboxSelected.length > 0 && textSearch !== "") {
        checkboxSelected.map(selected => {
            datos.push(...data.events.filter(evento => evento.name.toLowerCase().includes(textSearch.trim().toLowerCase()) && evento.category == selected))

        })
    }
    else if (checkboxSelected.length > 0 && textSearch === "") {
        checkboxSelected.map(selected => {
            datos.push(...data.events.filter(evento => evento.category == selected))
        })
    }
    else if (checkboxSelected.length == 0 && textSearch !== "") {
        datos.push(...data.events.filter(evento => evento.name.toLowerCase().includes(textSearch.trim().toLowerCase())))
    }
    else { datos.push(...data.events) }

    cartitasxd(datos)
}
arrayFiltrado()

function cartitasxd(eventito) {
    let templateHtml = ""
    eventito = eventito.sort((a, b) => a.date > b.date)
    console.log(eventito)
    if (eventito.length != 0) {
        for (let i = 0; i < eventito.length; i++) {
            if (data.currentDate < eventito[i].date) {
                templateHtml += `
                <div class="col pb-3">
                <div class="card h-100">
                <img src="${eventito[i].image} " class="card-img-top xd" alt=" ${eventito[i].name} " />
                <div class="card-body">
                <h5 class="card-title"> ${eventito[i].name} </h5>
                <p class="card-text">Description: ${eventito[i].description} </p>
                </div>
                <div class="card-footer d-flex align-items-center justify-content-between">
                <small class="text"> Price: $${eventito[i].price} / ${eventito[i].date} </small>
                <a href="./details.html?id=${eventito[i]._id} " class="btn boton-rosita btn-sm">See More</a>
                </div>
                </div>
                </div>
                `
            }
            document.querySelector("#cartinias").innerHTML = templateHtml
        }
    }
    else {
        document.querySelector("#cartinias").innerHTML = `
        <div class="d-flex flex-column pt-2 justify-content-center align-items-center">
            <h2> Event not found😎</h2>
            <img src="data:image/jpeg;base64," alt="xd">
            
        </div>
            `
    }
}

//NOTE Viejardo lo de abajo 👨‍🦳
/* function cartitasxd(datos){
    let contenCartas = document.querySelector(".cartinias");
    let arrayeventos = data.events;
    arrayeventos.forEach(datos => {
        if (data.fechaActual < datos.date) {
            contenedor.innerHTML = `
            <div class="card h-100">
            <img src="${datos.image}" class="card-img-top xd" alt="${datos.name} " />
            <div class="card-body">
            <h5 class="card-title">${datos.name}</h5>
            <p class="card-text">Description: ${datos.description}</p>
            </div>
            <div class="card-footer d-flex align-items-center justify-content-around">
            <small class="text">Price: $${datos.price} / ${datos.date}</small>
            <a href="/html/details.html" class="btn boton-rosita btn-sm">See more</a>
            </div>
            </div>`
            contenCartas.appendChild(contenedor);
        }
    }
)}; */
/* function displayCardEventos(data) {
    let templateHtml = ""
    for(i = 0; i < data.length; i++){
        templateHtml += `
    <div class="col pb-3">
        <div class="card h-100">
            <img src="${data[i].image} " class="card-img-top xd" alt=" ${data[i].name} " />
            <div class="card-body">
                <h5 class="card-title"> ${data[i].name} </h5>
                <p class="card-text">Description: ${data[i].description} </p>
            </div>
            <div class="card-footer d-flex align-items-center justify-content-between">
                <small class="text"> Price: $${data[i].price}</small>
                <a href="/html/details.html?id=${data[i].id} " class="btn boton-rosita btn-sm">See More</a>
            </div>
        </div>
    </div>
    `
    }
    document.querySelector("#cartinias").innerHTML = templateHtml
}
 */
//ANCHOR viejardo
/* 
function cartitasxd(){
    let contenCartas = document.querySelector(".cartinias");
    let arrayeventos = data.events;
    arrayeventos.forEach(element => {
        if (data.fechaActual < element.date) {
            let contenedor = document.createElement("div")
            contenedor.classList.add("col", "pb-3")
            contenedor.innerHTML = `
            <div class="card h-100">
            <img src="${element.image}" class="card-img-top xd" alt="${element.name} " />
            <div class="card-body">
            <h5 class="card-title">${element.name}</h5>
            <p class="card-text">Description: ${element.description}</p>
            </div>
            <div class="card-footer d-flex align-items-center justify-content-around">
            <small class="text">Price: $${element.price} / ${element.date}</small>
            <a href="/html/details.html" class="btn boton-rosita btn-sm">See more</a>
            </div>
            </div>`
            contenCartas.appendChild(contenedor);
        }
    }
)};
cartitasxd()
 */
/* 
//ANCHOR CATEGORIAS 
let categorias = arrayeventos.map(e => e.category).filter((element, position, arrayeventos) =>{
    return position === arrayeventos.indexOf(element);
})
console.log(categorias)
let contenCategorias = document.querySelector(".Category")
function categoriasxd(){
    contenCategorias.innerHTML=""
    categorias.forEach(element => {
        let contenedor = document.createElement("DIV");
        contenedor.classList.add("form-check","form-check-inline")
        contenedor.innerHTML=`
            <input class="form-check-input" type="checkbox" id="${element}" value="option1" />
            <label class="form-check-label" for="${element}">${element}</label>
        `
        contenCategorias.appendChild(contenedor)
    })
}
 */